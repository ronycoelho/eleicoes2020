...
# https://eleicoes.uol.com.br/2012/raio-x/partidos-e-prefeitos/numero-de-prefeitos-eleitos
#prefs.pt <- 
  
dt.2012.2016 %>% 
  filter(descricao_cargo == "PREFEITO",
         desc_sit_tot_turno == "ELEITO") %>% 
  count(ano_eleicao, sigla_partido, desc_sit_tot_turno, sort = T ) %>% 
  mutate(order = ifelse(ano_eleicao == 2016, n, 0),
         sigla_partido = fct_reorder(sigla_partido, order)) %>%
  arrange(desc(order)) %>% 
  ggplot(aes(x = n, y = sigla_partido, fill = as.factor(ano_eleicao)))+
  geom_col(position = "dodge")+
  theme_minimal()+
  theme(aspect.ratio = 1.3/1)+
  labs("Número de prefeituras por partido")
```
